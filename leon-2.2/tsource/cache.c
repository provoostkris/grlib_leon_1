...
//	IVALMSK = (0x03 << ((((int)line2)>>2)&(ILINESZ-1)));
	...
	/* idata parity test */
	  ...
	  line2();
	  if (((lr->ectrl >> IDE_BIT) & 3) != 1) fail(3);
	  
	/* itag parity test */
	  setitag((int *)line2,
	  	getitag((int *)line2) & ~(1<<8) & ~(1<<31));
	  lr->ectrl &= ~CPTE_MASK;
	  setidata((int *)line2,0);
	  line2();
	  if (((lr->ectrl >> ITE_BIT) & 3) != 1) fail(4);
	}
	...
	*ll = mrl[0];
	...
	/* ddata parity test */
	  ...
	  if (mrx[0] != 5) fail(10);
	  if (((lr->ectrl >> DDE_BIT) & 3) != 1) fail(11);
	/* dtag parity test */
	  lr->ectrl &= ~CPTE_MASK;
	  setddata(&mrx[0],0);
	  lr->ectrl |= CPTE_MASK;
	  ...
	  lr->ectrl &= ~CPTE_MASK;
	  while (lr->ectrl & CPTE_MASK) {}; /* avoid race condition */
	  if (mrx[0] != 5) fail(12);
	  if (getddata(&mr[0]) != 5) fail(13);
	  if (((lr->ectrl >> DTE_BIT) & 3) != 1) fail(14);
